...
function DeleteCommentModal({toggleDeleteCommentModal}) {
    const {currentComment, isDeletingComment} = useSelector(state => state.articles);
    const dispatch = useDispatch();

    const handleDeleteComment = async () => {
        await dispatch(actions.articles.deleteComment(currentComment));
            ...
            <div className="flex items-end justify-center min-h-screen pt-4 px-4 pb-20 text-center sm:block sm:p-0">
                ...
                <div
                    ...
                    <div className="mt-5 sm:mt-4 sm:flex sm:flex-row-reverse">
                        <button type="button"
                                className="w-full sm:w-20 inline-flex justify-center rounded-md border border-transparent shadow-sm px-4 py-2 bg-red-600 text-base font-medium text-white hover:bg-red-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 sm:ml-3 sm:text-sm"
